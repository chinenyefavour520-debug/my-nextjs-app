// boutique-frontend/app/(admin)/products/add/page.js
...
export default function AddProductPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post(API.ADMIN.PRODUCTS.ADD, payload);
      if (res.data.success) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow p-6 space-y-4">
          <div className="flex justify-between items-center border-b pb-2">
            <h3 className="font-semibold text-gray-700">Variants (Size / Color / Stock)</h3>
            <button 
              type="button" 
              onClick={addVariant}
          ...
          {variants.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-4">
              No variants added. Click "Add Variant" to add sizes or colors.
            </p>
          ) : variants.map((v, idx) => (
